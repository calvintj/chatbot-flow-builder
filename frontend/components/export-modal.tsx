...
export function ExportModal({ isOpen, onClose, data, onImportData }: ExportModalProps) {
  ...
  const handleDownload = () => {
    ...
    a.href = url
    a.download = `conversational-flow-${new Date().toISOString().split("T")[0]}.json`
    ...
    try {
      ...
      for (const step of parsedData) {
        if (!step.step_id || !step.instruction || !step.answer_options) {
          throw new Error("Each step must have step_id, instruction, and answer_options")
        }
      }
  ...
  const getDataStats = () => {
    ...
    const totalOptions = data.reduce((sum, step) => sum + Object.keys(step.answer_options).length, 0)
    const totalExamples = data.reduce((sum, step) => {
      return (
        sum +
        Object.values(step.few_shot_examples || {}).reduce((exSum: number, examples: any) => {
          return exSum + (Array.isArray(examples) ? examples.length : 0)
        }, 0)
      )
    }, 0)
    const totalConnections = data.reduce((sum, step) => sum + Object.keys(step.transitions || {}).length, 0)
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
          ...
          <TabsContent value="export" className="flex-1 flex flex-col space-y-4 min-h-0 pt-4"> {/* Added pt-4 for spacing */}
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="text-center p-3 bg-muted rounded-lg">
                <div className="text-2xl font-bold text-secondary">{stats.totalOptions}</div>
                <div className="text-xs text-muted-foreground">Options</div>
              </div>
              <div className="text-center p-3 bg-muted rounded-lg">
                <div className="text-2xl font-bold text-accent">{stats.totalConnections}</div>
                <div className="text-xs text-muted-foreground">Connections</div>
              </div>
              <div className="text-center p-3 bg-muted rounded-lg">
                <div className="text-2xl font-bold text-chart-1">{stats.totalExamples}</div>
          ...
          <TabsContent value="import" className="flex-1 flex flex-col space-y-4 min-h-0 pt-4"> {/* Added pt-4 for spacing */}
            <Alert>
              ...
              <AlertDescription>
                Import a JSON file to replace the current conversational flow. Make sure the data follows the correct
                format.
              </AlertDescription>
            </Alert>
                ...
                <div key={step.step_id} className="border rounded-lg p-4 space-y-3">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {Object.entries(step.answer_options).map(([key, value]) => (
                        <div key={key} className="flex items-center gap-2 text-xs">
                          ...
                          {step.transitions?.[key] && (
                            <>
                              ...
                              <Badge variant="secondary">{step.transitions[key]}</Badge>
                            </>
                          )}
                        </div>
                      ))}
